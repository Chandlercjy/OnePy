...
class indicatorBase(object):
    ...
    def _set_feed(self,marketevent):
        ...
        self.bar_list = m.info['bar_dict'][m.instrument]
    ...
    def _check_minperiod(self,minperiod,index):
        """直接将preload的dict一个一个插到bardict前面，然后开始计算"""
        """若preload_bar_list为空或者不够，则前进"""

        self.preload_limit = self.preload_bar_list[:minperiod-index]
        if len(self.preload_limit) >= minperiod-index:
            [self.bar_list.insert(0,i) for i in self.preload_limit]         # load to bar_list
            return True
        elif len(self.preload_limit) < (minperiod):
            return False
...
class indicator(indicatorBase):
    ...
    def SimpleMovingAverage(self,period,index=-1):
        minperiod = period
        if self._check_minperiod(minperiod,index):
            if index is 0:
                """开始计算指标"""
                data = [i['close'] for i in self.bar_list][-minperiod+index:]
