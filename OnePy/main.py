...
class Depreciated(object):
    ...
    def __check_finish_backtest(self):
        """检查回测是否结束"""
        backtest = [
            i.continue_backtest for i in self.feed_list]  # if finish, sum(backtest) = 0 + 0 + 0 = 0 -> False
    ...
    def set_backtest(self, feed_list, strategy_list, portfolio, target="Forex"):
        ...
        if target not in ["Forex", "Futures", "Stock"]:
            raise SyntaxError(
                "Target should be one of 'Forex','Futures','Stock'")
    ...
    def __output_summary(self):  # After
        ...
        d["Total_return"] = round(
            d["Final_Value"] / self.fill.initial_cash - 1, 5)
