...
class MyStrategy(op.StrategyBase):
    ...
    def next(self):
        ...
        if self.i.SMA(period=10, index=-1) > self.i.SMA(period=30,index=-1):
            self.Buy(0.1,stop=self.pct(1),limit=self.pips(100))
            if self.unre_profit[-1] <= 0:
                self.Buy(0.2,limit=self.pips(100))
            if self.unre_profit[-1] > 100:
                self.Exitall()
        # else:

        # else:
            # self.Sell(0.2,limit =self.pips(50),stop=self.pct(0.5))

go = op.OnePiece()

Forex = op.Forex_CSVFeed(datapath='data/EUR_USD30m.csv',instrument='EUR_USD',
                        fromdate='2012-04-01',todate='2012-05-02',
                         timeframe=1)
